...
|`phase-07b-ai-router.md`                         |7b   |Central AI router — model picker, sensitivity rules, retrofit Phases 1-7|1       |Phases 1-7 pushed|
...
**Total estimated sessions: 17-23**
...
1. **No sensitive data to external APIs.** All AI inference uses Ollama (localhost:11434) only
1. **Co-signatures are non-negotiable.** Two different officers required for every disbursement — enforce at API level, not just UI
1. **Executive session = no recording.** Events with exec session keywords get title/body/attendees sanitized before reaching any agent
1. **Admin sees metrics, never secrets.** Config show and admin dashboard must never expose pronouns, overwhelm_triggers, never_do instructions, .env values, or memory contents
1. **8 agents, not 3.** Don't hardcode for 3. The system must handle all 8 from day one
1. **agent_id is permanent.** Format: {role-slug}-{name-slug}-{4char}. Never change it after creation — it's the key for memory, volumes, and logs
